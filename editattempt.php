...
if(isset($_POST['btn-editattempt']))
{
	...
	$sql = "
		update Attempt 
		set IdSession = $IdSession, IdRoute = $IdRoute, Result = $Result, Percentage = $Percentage, Comment = '$Comment'
		where Id = $IdAttempt
	";
...
$sql = "
select IdSession, IdRoute, Result, Percentage, Comment
from Attempt
where Id = $IdAttempt
";
...
<table class="noborder align="center" width="30%" border="0">
	<tr><td class="noborder"><label><h1>Edit attempt</h1></label></td></tr>
	<tr>
		<td class="noborder">
			<select name="IdSession" onChange="RefreshSession(this.value, <?php echo $IdSession ?>)" required>
...
<br>
<button style="width:100px" type="submit" name="btn-editattempt">OK</button>&nbsp;&nbsp;
...
<?php
if (!empty($IdSession))
	echo 'RefreshSession(' . $IdSession . ', ' . $IdRoute . ');';
?>

ResultChanged();

function RefreshSession(idSession, idRoute)
{
	RefreshPartners(idSession);
	RefreshRoutes(idSession, idRoute);
}
...
function RefreshRoutes(idSession, idRoute)
{
	...
	xmlhttpRoutes.open("GET", "routeSelector.php?includeRemoved=1&idSession=" + idSession + "&idRoute=" + idRoute, true);
	xmlhttpRoutes.send();
}
